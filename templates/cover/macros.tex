...
{% macro authors(authors) %}
    \begin{varwidth}{->.98\textwidth}\Large{\textsc{->{{ authors | join(sep=" \&~") }}.}}\end{varwidth};
{% endmacro authors %}


{% macro nodes(nodes, suffix, info, txt_opacity, fill_opacity) %}
    {% for i in range(start=1,end=3) %}
        ...
        {% if i == 1 %}
            ...
            {% set nex = nodes.2 %}
        {% elif i == 2 %}
            {% set bd = self::authors(authors=info.authors) %}
            {% set con = info.authors | length > 0  %}
            {% set act = nodes.2 %}
            {% set nex = nodes.3 %}
        {% elif i == 3 %}
            {% set bd = self::translators(translators=info.translators) %}
            {% set con = info.translators | length > 0  %}
            {% set act = nodes.3 %}
            {% set nex = nodes.4 %}
        {% endif %}
